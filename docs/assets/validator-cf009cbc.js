import{a as e,o as d,j as n}from"./index-9017f177.js";const o={class:"markdown-body"},a=n(`<h2>Validator</h2><blockquote><p>1.5.0 新增</p></blockquote><p>校验器，用于对表单进行验证，并提示相应的错误信息。</p><h3>示例</h3><ul><li><p>基本用法</p><p>Validator 作为一个独立的组件，通过 model 属性绑定它需要检验的数据；rules 定义验证规则，在 rules 规则中，可以配置 type、required 等内置规则，也可以用 pattern 添加校验的正则表达式，或 custom 自定义验证函数，具体细节可见后面的规则；针对各类规则，我们有比较完善的默认提示文案，具体可见后面的默认提示文案；同时，你也可以用 messages 属性用于自定义提示信息。</p><pre><code class="language-html">&lt;cube-input v-model=&quot;text&quot; placeholder=&quot;E-mail&quot;&gt;&lt;/cube-input&gt;
