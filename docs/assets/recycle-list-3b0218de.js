import{a as e,o,j as d}from"./index-9017f177.js";const l={class:"markdown-body"},s=d(`<h2>RecycleList</h2><blockquote><p>1.12.0 新增</p></blockquote><p>可回收的滚动列表，始终将 DOM 的数量控制在一个很低的范围。在海量数据的渲染列表中，拥有卓越的性能。</p><h3>注意事项</h3><ul><li><p>首先要实现无限滚动，必须保证要有一个固定高度的父容器元素包裹组件。详细可以参考示例。</p></li><li><p>如果列表内容存在不固定宽高的图片，由于图片加载的机制，每个 item 的 DOM 渲染了，图片可能还未加载，导致每个 item 的位置计算错误，所以作为调用方，需要将未加载的图片用样式固定住，这样就能避免高度计算错误。</p></li><li><p>内部的实现依赖 Promise，所以你需要引入 Promise polyfill 来做兼容。</p></li></ul><h3>示例</h3><ul><li><p><strong>1. 默认使用</strong></p><pre><code class="language-html">&lt;div class=&quot;view-wrapper&quot;&gt;
