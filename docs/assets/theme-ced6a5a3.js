import{a as r,o as e,j as c}from"./index-9017f177.js";const n={class:"markdown-body"},l=c(`<h2>主题</h2><p>移动端的组件对主题定制的需求会更多，从 <code>cube-ui^1.0.2</code> 开始，通过 webpack 工具可以实现自定义主题样式。</p><p><strong>注意：</strong> 自定义主题依赖<a href="#/zh-CN/docs/post-compile">后编译</a>。</p><h3>主题文件</h3><p>在你的项目的 <code>src/</code> 目录下创建一个 <code>theme.styl</code> 的文件，内容如下（展示的全是默认样式，你可以选择这针对不同组件做定制）：</p><pre><code class="language-stylus">// 如果你需要使用 cube-ui 自带的颜色值 需要 require 进来
...
</code></pre><h3>配置 webpack</h3><p>如果你的项目是用 vue-cli 生成的，那么找到 <code>build/utils.js</code> 中的 <code>exports.cssLoaders</code> 函数，改成如下：</p><pre><code class="language-js">exports.cssLoaders = function (options) {
  options = options || {}
...
</code></pre><h3>结果</h3><p>这样修改自定义的主题文件内容，最终编译打包后的样式就是你需要的；而且这样也不存在样式冗余覆盖的问题，因为直接是对组件样式的变量做的修改，而不是采用传统的样式覆盖。</p>`,11),t=[l],d={__name:"theme",setup(i,{expose:o}){return o({frontmatter:{}}),($,s)=>(e(),r("div",n,t))}};export{d as default};
