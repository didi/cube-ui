import{a as e,o as l,j as a}from"./index-e8a96955.js";const d={class:"markdown-body"},o=a(`<h2>TabBar</h2><blockquote><p>1.10.0 新增</p></blockquote><p>选项卡。</p><h3>示例</h3><p>如下示例相关代码在<a href="https://github.com/didi/cube-ui/tree/master/example/pages/tab-bar">这里</a>。</p><h3>CubeTabBar</h3><p>支持默认的点击高亮效果，又支持下划线跟随的效果，并且支持自定义的插槽，实现icon与label搭配的类似于app底部选项卡的样式。</p><ul><li><p>默认样式</p><p>传入如下 <code>tabs</code> 的数据结构便能初始化 <code>cube-tab-bar</code>，必须使用 <code>v-model</code> 指令来选中对应的 tab， v-model 的参数的值必须与某一项 tab 的 label 属性对应（1.12.5 版本后是和 value 属性对应），icon 属性是用做于 class 选择器，一般是用字体图标样式，<code>cube-tab-bar</code> 在不同的时机派发 <code>click</code> 与 <code>change</code> 事件，参数则是每次选中的 tab 对应的 label 值（1.12.5 版本后是 value 值）。</p><pre><code class="language-html">&lt;template&gt;
  &lt;cube-tab-bar
