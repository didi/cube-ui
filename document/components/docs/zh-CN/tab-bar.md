...
- 默认样式
  ...
  传入如下`tabs`的数据结构便能初始化`cube-tab-bar`,必须使用`v-model`指令来选中对应的tab，v-model的参数的值必须与某一项tab的label属性对应,并且不同的时机派发`click`与`change`事件，参数则是每次选中的tab对应的label值
...
- `cube-tab-bar` 事件
  ...
  | 事件名 | 说明 | 参数1 |
  | - | - | - | - |
  | click | 当tab被点击时派发 | 点中的tab的label值 |
  | change | 当点击不同的tab时派发 | 点中的tab的label值 |
