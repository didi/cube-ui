import "./css.mjs";
import { withDirectives as dt, openBlock as _, createElementBlock as M, normalizeClass as Pt, normalizeStyle as ne, createElementVNode as b, withModifiers as Z, renderSlot as kt, vShow as mt, resolveComponent as oe, createBlock as bt, Transition as Yt, withCtx as rt, createVNode as J, toDisplayString as Xt, createCommentVNode as me, Fragment as gt, renderList as vt, resolveDynamicComponent as ge, mergeProps as re, camelize as ve, h as Dt, render as It, isVNode as Ct } from "vue";
/*!
  ...
  e.prototype.on = function(t, i) {
    ...
    if (i)
      for (var s = i.length, n = [].concat(Te(i)), o = 0; o < s; o++) {
        var r = n[o], a = ye(r, 2), h = a[0], l = a[1];
        h && h.apply(l, [].slice.call(arguments, 1));
      }
  };
}
var Y = typeof window < "u", Q = Y && navigator.userAgent.toLowerCase(), xe = Q && /wechatdevtools/.test(Q), _e = Q && Q.indexOf("android") > 0;
function P() {
...
var xt = Y && document.createElement("div").style, at = function() {
  if (!Y)
    return !1;
  var e = {
...
var he = k("transform"), we = Y && k("perspective") in xt, Tt = Y && ("ontouchstart" in window || xe), Se = he !== !1, Me = Y && k("transition") in xt, x = {
  transform: he,
...
var lt = function() {
  ...
  function(e) {
    return window.setTimeout(e, (e.interval || Ie) / 2);
  } : ce;
}(), X = function() {
...
function Ce(e) {
  e.prototype._start = function(t) {
    ...
    if (!(!this.enabled || this.destroyed || N[t.type] !== this.initiated)) {
      ...
      if (this.directionX = n > 0 ? pt : n < 0 ? ct : 0, this.directionY = o > 0 ? j : o < 0 ? W : 0, !(this.options.pullDownRefresh && this._checkPullDown())) {
        ...
        if (!this.resetPosition(this.options.bounceTime, T.bounce)) {
          ...
          if (this.options.momentum && r < this.options.momentumLimitTime && (h > this.options.momentumLimitDistance || a > this.options.momentumLimitDistance)) {
            ...
            var m = this.directionX === pt && d || this.directionX === ct && f ? this.wrapperWidth : 0, y = this.directionY === j && u || this.directionY === W && p ? this.wrapperHeight : 0, E = this.hasHorizontalScroll ? $t(this.x, this.startX, r, this.maxScrollX, this.minScrollX, m, this.options) : { destination: i, duration: 0 }, S = this.hasVerticalScroll ? $t(this.y, this.startY, r, this.maxScrollY, this.minScrollY, y, this.options) : { destination: s, duration: 0 };
          ...
          if (this.options.snap) {
            var A = this._nearestSnap(i, s);
            this.currentPage = A, l = this.options.snapSpeed || Math.max(Math.max(Math.min(Math.abs(i - A.x), 1e3), Math.min(Math.abs(s - A.y), 1e3)), 300), i = A.x, s = A.y, this.directionX = 0, this.directionY = 0, ot = this.options.snap.easing || T.bounce;
          }
          if (i !== this.x || s !== this.y) {
            (i > this.minScrollX || i < this.maxScrollX || s > this.minScrollY || s < this.maxScrollY) && (ot = T.swipeBounce), this.scrollTo(i, s, l, ot);
            return;
          }
      ...
      if (this.options.wheel) {
        ...
        if (!i) {
          ...
          if (o && this.lastClickTime) {
            ...
            P() - this.lastClickTime < h && (r = !0, Pe(t));
          }
          return this.options.tap && Ee(t, this.options.tap), this.options.click && !K(t.target, this.options.preventDefaultException) && le(t), this.lastClickTime = r ? null : P(), !0;
        }
        return !1;
      }
    return !1;
    ...
    var t = this;
    this.enabled && (_e && (this.wrapper.scrollTop = 0), clearTimeout(this.resizeTimeout), this.resizeTimeout = setTimeout(function() {
      t.refresh();
    }, this.options.resizePolling));
  }, e.prototype._startProbe = function() {
    X(this.probeTimer), this.probeTimer = lt(i);
    var t = this;
    function i() {
    ...
    if (G(!F(t) && !F(i), "Translate x or y is null or undefined."), F(s) && (s = this.scale), this.options.useTransform ? this.scrollerStyle[x.transform] = "translate(" + t + "px," + i + "px) scale(" + s + ")" + this.translateZ : (t = Math.round(t), i = Math.round(i), this.scrollerStyle.left = t + "px", this.scrollerStyle.top = i + "px"), this.options.wheel)
    ...
    function d() {
      ...
      o._translate(m, y, E), o.isAnimating && (o.animateTimer = lt(d)), o.options.probeType === H && o.trigger("scroll", {
        x: o.x,
        y: o.y
      });
    }
  ...
  e.prototype._initSnap = function() {
    ...
    typeof n == "string" && (n = this.scroller.querySelectorAll(n)), this.on("refresh", function() {
      ...
      if (i.loop) {
        ...
        o.length > 2 && (At(t.scroller, o[o.length - 1]), At(t.scroller, o[0]));
      }
    });
...
function We(e) {
  e.prototype._initPullDown = function() {
    ...
    return this.directionY !== j || this.y < s ? !1 : (this.pulling || (this.pulling = !0, this.trigger("pullingDown")), this.scrollTo(this.x, o, this.options.bounceTime, T.bounce), this.pulling);
...
function He(e) {
  e.prototype._initZoom = function() {
    ...
    this.scale = Math.min(Math.max(s, o), a), this.setScale(this.scale), this.scrollerStyle[x.transformOrigin] = "0 0";
    ...
    this.startDistance = Lt(n, o), this.startScale = this.scale;
    ...
    if (!(!this.enabled || this.destroyed || N[t.type] !== this.initiated)) {
      this.options.preventDefault && t.preventDefault(), this.options.stopPropagation && t.stopPropagation();
      var i = t.touches[0], s = t.touches[1], n = Math.abs(i.pageX - s.pageX), o = Math.abs(i.pageY - s.pageY), r = Lt(n, o), a = r / this.startDistance * this.startScale;
      this.scaled = !0;
...
var Nt = 30, Ht = 10, U = 200, pe = 2e3;
function Oe(e) {
  e.prototype._initInfinite = function() {
    this.options.probeType = 3, this.maxScrollY = -pe, this.infiniteScroller = new v(this, this.options.infinity);
  };
}
function _t(e) {
  if (e && e.classList)
...
v.prototype.onScroll = function() {
  ...
  t < 0 ? (s -= Nt, n += Ht) : (s -= Ht, n += Nt), this.fill(s, n), this.maybeRequestContent();
};
...
v.prototype._removeTombstones = function() {
  for (var e = void 0, t = 0, i = this.items.length, s = 0; s < i; s++) {
    ...
    (!n || _t(n)) && !o && (e || (e = s), n && this.scrollerEl.removeChild(n));
  }
...
v.prototype._collectUnusedNodes = function() {
  for (var e = [], t = 0; t < this.items.length; t++) {
    ...
    i && (_t(i) ? (this.tombstones.push(i), this.tombstones[this.tombstones.length - 1].style.display = "none") : e.push(i)), this.items[t].node = null;
  }
  return e;
};
...
v.prototype._setupAnimations = function(e, t) {
  ...
  this.scroller.maxScrollY = -(t - this.wrapperEl.offsetHeight + (this.hasMore ? pe : 0)), setTimeout(function() {
    for (var h in e) {
      var l = e[h];
...
const nt = (e, t) => {
  ...
  for (const [s, n] of t)
    i[s] = n;
  return i;
}, Re = "cube-popup", Rt = "mask-click", Ve = {
  name: Re,
  mixins: [it, st],
  emits: [Rt],
  props: {
  ...
  methods: {
    maskClick(e) {
      this.$emit(Rt, e), this.maskClosable && this.hide();
    }
  }
}, Ue = {
  key: 0,
...
function qe(e, t, i, s, n, o) {
  return dt((_(), M("div", {
    class: Pt(["cube-popup", o.rootClass]),
    style: ne({ "z-index": e.zIndex })
  }, [
    b("div", {
      class: "cube-popup-mask",
      onTouchmove: t[0] || (t[0] = Z(() => {
      }, ["prevent"])),
    ...
    b("div", {
      class: Pt(["cube-popup-container", o.containerClass]),
      onTouchmove: t[2] || (t[2] = Z(() => {
      }, ["prevent"]))
    }, [
      e.$slots.default ? (_(), M("div", Ue, [
        kt(e.$slots, "default")
      ])) : (_(), M("div", {
        key: 1,
        class: "cube-popup-content",
...
const Ke = /* @__PURE__ */ nt(Ve, [["render", qe]]), ut = {
  value: "value",
  text: "text",
  order: "order"
}, ue = {
  ...
  computed: {
    valueKey() {
      return this.alias.value || ut.value;
    },
    textKey() {
      return this.alias.text || ut.text;
    },
    orderKey() {
      return ut.order;
    },
...
const St = fe("Undefined"), Be = fe("Number");
function ti(e, t, i, s) {
  const n = {
    year: "(Y+)",
    month: "(M+)",
  ...
  if (new RegExp(n[e], s).test(t)) {
    const o = e === "year" ? i.toString().substr(4 - RegExp.$1.length) : RegExp.$1.length === 1 ? i : ei(i);
...
function ii(e, t) {
  ...
  for (const s in i)
    t = ti(s, t, i[s].value, i[s].regExpAttributes);
  return t;
}
let $;
const Vt = "zh-CN", z = {
  name: "locale",
  install(e) {
    z.installed || ($ = e.config.globalProperties, $.$cubeLang = Vt, $.$cubeMessages = { [Vt]: je, "en-US": Ge }, z.installed = !0);
  },
  use(e, t) {
    $.$cubeLang = e;
  ...
  helpers: {
    toLocaleDateString(e, t) {
      const i = Be(e) ? e : e.replace(/-/g, "/"), s = new Date(i);
      return St(t) ? s.toDateString() : ii(s, t);
    }
  },
  addHelper(e, t) {
    e in z.helpers || (z.helpers[e] = t);
  }
}, de = z, si = "Translation is not registered correctly, you can call Locale.use() to install it.", Mt = {
  computed: {
    $t() {
      const e = this.$cubeLang, t = this.$cubeMessages[e];
      return St(t) ? (Ze(si), "") : (i) => Je(t, i);
    }
  },
  beforeCreate() {
    de.install(this._.appContext.app);
  }
}, ni = typeof window < "u", V = ni && navigator.userAgent.toLowerCase();
V && V.indexOf("android") > 0;
const oi = V && /iphone|ipad|ipod|ios/.test(V);
function ri() {
  let e = !0;
  if (oi) {
    const t = Qe(V);
...
const Ut = ri();
const ai = "cube-picker", Ft = "select", qt = "value-change", Kt = "cancel", jt = "change", hi = {
  name: ai,
  mixins: [it, st, ue, wt, Mt],
  emits: [Ft, qt, Kt, jt],
  props: {
    pending: {
      type: Boolean,
  ...
  methods: {
    confirm() {
      ...
      this.$emit(Ft, this._values, this._indexes, t), e && this.$emit(qt, this._values, this._indexes, t);
    },
    maskClick() {
      this.maskClosable && this.cancel();
    },
    cancel() {
      this.hide(), this.$emit(Kt);
    },
    show() {
      if (!this.isVisible)
    ...
    _createWheel(e, t) {
      if (this.wheels[t])
        ...
        const i = this.wheels[t] = new w(e.children[t], {
          wheel: {
        ...
        i.on("scrollEnd", () => {
          this.$emit(jt, t, this._getSelectIndex(i));
        });
      }
    ...
    _getSelectIndex(e) {
      const t = e.y;
      let i;
      return Ut ? i = e.getSelectedIndex() : t > e.minScrollY ? i = 0 : t < e.maxScrollY ? i = e.items.length - 1 : i = Math.round(Math.abs(t / e.itemHeight)), i;
    }
  },
  beforeUnmount() {
...
}, li = { class: "cube-picker-choose border-bottom-1px" }, ci = { class: "cube-picker-title-group" }, pi = ["innerHTML"], ui = ["innerHTML"], fi = { class: "cube-picker-content" }, di = /* @__PURE__ */ b("i", { class: "border-bottom-1px" }, null, -1), mi = /* @__PURE__ */ b("i", { class: "border-top-1px" }, null, -1), gi = {
  class: "cube-picker-wheel-wrapper",
  ref: "wheelWrapper"
}, vi = { class: "cube-picker-wheel-scroll" }, yi = ["innerHTML"], Ti = /* @__PURE__ */ b("div", { class: "cube-picker-footer" }, null, -1);
function bi(e, t, i, s, n, o) {
  const r = oe("cube-popup");
  return _(), bt(Yt, { name: "cube-picker-fade" }, {
    default: rt(() => [
      dt(J(r, {
        type: "picker",
        mask: !0,
        center: !1,
        "z-index": e.zIndex,
        onTouchmove: t[3] || (t[3] = Z(() => {
        }, ["prevent"])),
        onMaskClick: o.maskClick
      }, {
        default: rt(() => [
          J(Yt, { name: "cube-picker-move" }, {
            default: rt(() => [
              dt(b("div", {
                ...
                b("div", fi, [
                  ...
                  mi,
                  b("div", gi, [
                    (_(!0), M(gt, null, vt(n.finalData, (a, h) => (_(), M("div", {
                      key: h,
                      style: ne({ order: o._getFlexOrder(a) })
                    }, [
                      b("ul", vi, [
                        (_(!0), M(gt, null, vt(a, (l, u) => (_(), M("li", {
...
const L = /* @__PURE__ */ nt(hi, [["render", bi]]), xi = "cube-cascade-picker", Gt = "select", Zt = "cancel", Jt = "change", _i = {
  name: xi,
  mixins: [it, st, ue, wt, Mt],
  emits: [Gt, Zt, Jt],
  props: {
  ...
  methods: {
    ...
    _pickerChange(e, t) {
      t !== this.pickerSelectedIndex[e] && (this.pickerSelectedIndex.splice(e, 1, t), this.async ? this.pending = e !== this.pickerData.length - 1 : this._updatePickerData(e + 1)), this.$emit(Jt, e, t);
    },
...
const O = /* @__PURE__ */ nt(_i, [["render", wi]]), Si = "cube-segment-picker", Qt = "next", Bt = "prev", te = "select", ee = "cancel", ie = "change", Mi = {
  name: Si,
  mixins: [it, st, wt, Mt],
  emits: [Qt, Bt, te, ee, ie],
  props: {
    data: {
      type: Array,
      default() {
  ...
  methods: {
    ...
    _select(...e) {
      this.selectedVal[this.current] = e[0], this.selectedIndex[this.current] = e[1], this.selectedText[this.current] = e[2], this.current < this.data.length - 1 ? (this.$emit(Qt, this.current, ...e), this.current++, this.currentPicker.show()) : (this.isVisible = !1, this.$emit(te, this.selectedVal, this.selectedIndex, this.selectedText), this.current = 0);
    },
    _cancel(...e) {
      this.current > 0 ? (this.$emit(Bt, this.current, ...e), this.current--, this.currentPicker.show()) : (this.isVisible = !1, this.$emit(ee));
    },
    _change(...e) {
      this.$emit(ie, this.current, ...e);
    }
  }
}, Ei = { class: "cube-segment-picker" };
function Pi(e, t, i, s, n, o) {
  return _(), M("div", Ei, [
    (_(!0), M(gt, null, vt(i.data, (r, a) => (_(), bt(ge(r.is || "cube-picker"), re({
      key: a,
      index: a,
      ref_for: !0,
      ref: "pickers",
      ...
      "confirm-txt": r.confirmTxt || (a === i.data.length - 1 ? e._confirmTxt : o._nextTxt),
      "cancel-txt": r.cancelTxt || (a === 0 ? e._cancelTxt : o._prevTxt),
...
const C = /* @__PURE__ */ nt(Mi, [["render", Pi]]);
let ki = 0;
const ft = [], Yi = (e, t, i = null, s = null) => {
  let n = t, o = i ? i(Dt) : null;
  const r = document ? document.createElement("div") : null, a = "cube_create_component_" + ki++, h = J({
    render() {
      return J(e, {
        ...n
        // ref: '$cre'
  ...
  s && (h.appContext = s), ft.push(h);
  let l;
  return r && (It(h, r), l = h.component.subTree.component.proxy, l.$remove = function(u) {
    It(null, r), e._instance = null, u && u(), r && document.body.contains(r) && document.body.removeChild(r);
    const p = ft.findIndex((d) => {
      const { id: f } = d.props;
      return a === f;
    });
    ~p && ft.splice(p, 1);
  }, l.$updateProps = function(u, p, d = !0) {
    n = { ...d ? n : {}, ...u }, o = p ? { ...o || {}, ...p(Dt) } : null, h.component.proxy.$forceUpdate();
  }, document.body.appendChild(r)), l;
};
function se(e) {
  if (!!this && !!this._ && Ct(this._.vnode)) {
    const i = this && this._ && Ct(this._.vnode) ? this._.vnode.props : null;
    this._.vnode.props = re(i || {}, {
...
function Et(e, t, i, s) {
  t.$create = e.config.globalProperties[`$create${ve(t.name.replace("cube-", "")).replace(/^\w/, (n) => n.toUpperCase())}`] = function(n, o = null) {
    if (s && t._instance)
      return n && t._instance.$updateProps(n, o, !1), se.call(this, t._instance), t._instance;
    const r = t._instance = Yi(t, n, o, this ? this._.appContext : null);
    return se.call(this, r), r;
  };
}
function Xi(e, t) {
  Et(e, t);
...
C.install = function(e) {
  e.component(L.name, L), e.component(O.name, O), e.component(C.name, C), de.install(e), Xi(e, L), Di(e, O), Ii(e, C);
};
C.Picker = L;
O.Picker = L;
C.CascadePicker = O;
