...
describe('Select.vue', () => {
  ...
  it('should render correct contents - update txt', (done) => {
    ...
    vm.$nextTick(() => {
      expect(vm.picker.$el.querySelector('.cube-picker-choose .cube-picker-cancel').textContent.trim())
        .to.equal('cancel2')
      expect(vm.picker.$el.querySelector('.cube-picker-choose .cube-picker-confirm').textContent.trim())
        .to.equal('confirm2')
  ...
  it('should trigger events', function (done) {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        const confirmBtn = document.querySelector('.cube-picker-choose .cube-picker-confirm')
        confirmBtn.click()
        expect(changeHandle)
          .to.be.callCount(1)
        done()
